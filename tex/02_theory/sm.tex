...
% TODO: Write more flowy intro.
% Lagrangian mechanics.
% * Gives equation of motion, \ie how the system evolves 
% Transformations can be applied but Lagrangian must remain invariant.
% Noether's Theorem says that an invariance (\ie a symmetry) implies the existence of an associated current that is \emph{conserved}.
% * One of the most profound realizations in physics was made ~\cite{Noether1918}.
...
The case where $\lambda > 0$ in Eqn.~\ref{eqn:scalar_pot} indicates that a minimum exists in the potential and that $\phi$ is self-coupling~\cite{bass_higgs_2021}.
...
\end{equation*}
where $\theta_W$ is the weak mixing angle and has been measured to be $\sin^2{\theta_W} = 0.223\,37 \pm 0.000\,10$~\cite{Sirlin:1980nh}.
...
Hence, from the initial four degrees of freedom of the Higgs field, two are absorbed by the $\PW^\pm$ gauge bosons, one by the $\PZ$ gauge boson, and there is one remaining degree of freedom, $\PH$, that is the physical Higgs boson---a relatively new and eponymous scalar particle dreamt of by Peter Higgs~\cite{PhysRevLett.13.321,PhysRev.145.1156}.
