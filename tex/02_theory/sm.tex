...
\end{equation*}
which becomes the more familiar Dirac equation when using natural units ($c\equiv \hbar \equiv 1$) and extending $\psi_{\Pe}(x) \to \psi(x)$ for the case of a free spin-1/2 Dirac field:
\begin{equation}
...
\begin{equation*}
    ...
    \quad ,
\end{equation*}
where $I_2$ is the $2 \times 2$ identity matrix and $\sigma^a$ indicates the Pauli matrices:
\begin{equation*}
    \sigma_1 = 
    \begin{pmatrix}
        I_2 & 0 \\
...
\end{align*}
where \lagrangdens is called the `Lagrangian density' and is related to \lagrang through:
\begin{equation*}
    \lagrang = \int{d^3x \, \lagrangdens}.
\end{equation*}
Gauge transformations can be applied to particle fields so long as \lagrangdens remains invariant.

In all its glory, the SM Lagrangian density is~\cite{Halzen:1984mc, thesis_xunwu}:
\begin{align*}
    \lagrangdens =
    & - \frac{1}{4} B_{\mu \nu} B^{\mu \nu} - \frac{1}{4} W^a_{\mu \nu} W^{a \mu \nu} - \frac{1}{4} G^k_{\mu \nu} G^{k \mu \nu}
    \\
    ...
    & + \sum_f{
        % \left[
            \bar{\psi}^f_R (i {\slashed D_\mu}) \psi_R^f + \bar{\psi}^f_L (i {\slashed D_\mu}) \psi_L^f
        % \right]
        }
    \\
    & + \sum_f{
...
\end{align*}
where the field strength tensors for the interaction fields ($B_\mu$, $W_\mu^a$, and $G_\mu^k$) of their corresponding group are:
\begin{align*}
    U(1) : \quad & B_{\mu \nu}    = \partial_\mu B_\nu - \partial_\nu B_\mu
    \\
...
\end{align*}
% TODO: Define these:
% where the covariant derivative is given by ${\slashed D_\mu} = \gamma^\mu D_\mu$,
% where $a$
% partial derivative, Dslash,
% Bnu, Wnua Gnuk, g, g_s, varepsilon^abc, f^klm,
% \Phi, \Phi^\dagger, \mu, \lambda,
% \psi_R^f, \bar{\psi}_R^f
% $\mu$ and $\nu$ are the Lorentz vector indices which run from 1--4,
% $a$ and $k$ are the group generator indices which run from 1--3 and 1--8, respectively,
% $D_\mu$ is the covariant derivative which contains the field strength coefficients ($g\prime$, $g$, and $g_s$)
% and the sums are taken over the fermion flavors $f$, and for each coupling there exists a coupling strength $g_f$.
