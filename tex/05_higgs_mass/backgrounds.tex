...
\begin{align}
	...
	&= \left( \effprpass \right)^2 
	\left[ \effnppass(\ell_1^k) \cdot \effnpfail(\ell_2^k) + \effnpfail(\ell_1^k) \cdot \effnppass(\ell_2^k) \right]
	\notag
	\\
	&= \left( \effprpass \right)^2
	\left[
...
and $\hat{w}^k_{\ZZ}$ accounts for the differential QCD and electroweak $K$ factors.
% TODO:defined in Sec.~\ref{sec:bkg_irred}.
% The plots in \figurename~\ref{FINAL RBE PLOTS} make use of Eq.~\ref{eqn:rbe_formula_final}.

% TODO:MOVE the line below to some place better?
