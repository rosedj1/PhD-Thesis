...
% Processes which pass the signal event selection but are not actually the signal process of interest (HZZ4L) are called background processes.
These background events pass the signal event selection (Section~\ref{subsubsec:zz_sel}) and thus spoil the purity of the signal events.
...
% \begin{itemize}
	% \item signal $(\HZZ)$,
% 	\item irreducible background (IB),
% 	\item reducible background (RB).
% \end{itemize}
...
\end{itemize}
The event selection for the 2P2F and 3P1F CRs is almost identical to that of the SR (Section~\ref{subsubsec:zz_sel}),
except that the FTS lepton(s) are required to build the $\PZ_{2}$.
The events that contribute most to the 2P2F CR are those that produce 2 prompt and 2 nonprompt leptons (\emph{2pr events})
and, similarly, events that contribute mostly to the 3P1F CR are those that produce 3 prompt and 1 nonprompt leptons (\emph{3pr events}).

To understand the logic behind the final formula for \fourPexp (Eq.~\ref{eqn:rbe_formula_final}), it is helpful to first analyze a single event:
...
% $\left( \epsilon^\mathrm{pr(np)}_\mathrm{P} + \epsilon^\mathrm{pr(np)}_\mathrm{F} = 1 \right)$,
\begin{equation*}
...
then Eq.~\ref{eqn:wt_2prto3p1f}  % TODO: Update with new equation number.
becomes
\begin{equation}
...
A smaller fraction of the time, both nonprompt leptons from a 2pr event may be misidentified as PTS leptons.
...
% If the total number of true 2pr events is $X_2$, then the expected number of 2P2F events $\left( \twoPtwoFexp \right)$ is obtained via Eq.:
\begin{align}
	\label{eqn:n2p2f_x2}
	\twoPtwoFexp
	...
	\notag
	\\
	% &= \sum_{k=1}^{X_{2}} \epsilon^2 \left( \effnpfail \right)^2
	% \\
	...
	% \\
	% \implies X_{2} &= \frac{ \twoPtwoFexp }{ \epsilon^2 \left( 1-f \right)^2 }
\end{align}

The strategy to relate $X_3$ to the observed number of 3P1F events $\left( \threePoneFexp \right)$ is not as straightforward as relating $X_2$ to \twoPtwoFexp,
