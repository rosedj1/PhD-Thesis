...
\Cref{sec:analyzed_data} describes the triggers used for this analysis to select events with the \fourl final state found in the corresponding data sets.
For each chosen event, the subdetectors of CMS (\crefrange{sec:tracker}{sec:muon_sys}) provide a plethora of track and energy-detection information to reconstruct \emph{objects}, which are representations of the underlying particles within the event.
...
\begin{figure}[ht]
    ...
    \begin{feynman}[large]
        ...
        \vertex [above = 2.5cm of gg] (g1) {$\Pg$};
        \vertex [below = 2.5cm of gg] (g2) {$\Pg$};
        \vertex [right = 1.5cm of gg] (ttt); 
        \vertex [above = 1.5cm of ttt] (t1); 
        \vertex [below = 1.5cm of ttt] (t2); 
        \vertex [right = 2cm of ttt] (tt); 
        \vertex [right = 2cm of tt] (h); 
	...
	\begin{tikzpicture}
		\begin{feynman}[large]
			\vertex (topleft); % h at beginning 
			\vertex [right=3cm of topleft] (topright); % h at beginning 
			\vertex [below=3cm of topleft] (bottomleft); % h at beginning 
			\vertex [below right=3cm and 3cm of topleft] (bottomright); % h at beginning 

			\vertex [above left=.75cm and 2.1cm of topleft] (gt) {\Pg}; % h at beginning 
			\vertex [below left=.75cm and 2.1cm of bottomleft] (gb) {g}; % h at beginning 

			\vertex [above right=.75cm and 1.5cm of topright] (Zt); % h at beginning 
			\vertex [below right=.75cm and 1.5cm of bottomright] (Zb); % h at beginning 
			\vertex [above right=.5cm and .4cm of topright] (Ztlabel) {Z}; % h at beginning 
			\vertex [below right=.5cm and .4cm of bottomright] (Zblabel) {Z}; % h at beginning 

			\vertex [above right=.75cm and 2.1cm of Zt] (e1) {$\ell^-$}; % h at beginning 
			\vertex [below right=.75cm and 2.1cm of Zt] (e2) {$\ell^+$}; % h at beginning 
			\vertex [above right=.75cm and 2.1cm of Zb] (e3) {$\ell^-$}; % h at beginning 
			\vertex [below right=.75cm and 2.1cm of Zb] (e4) {$\ell^+$}; % h at beginning 
			
			\diagram* {
				(gt) -- [gluon] (topleft),
				(gb) -- [gluon] (bottomleft),
				(topleft) -- [fermion, edge label=$\Pqt$] (topright) -- [fermion, edge label=$\Paqt$] (bottomright) -- [fermion, edge label=$\Paqt$] (bottomleft) -- [fermion, edge label=$\Pqt$] (topleft);
				(e2) -- [fermion] (Zt) -- [fermion] (e1);
				(e4) -- [fermion] (Zb) -- [fermion] (e3);
				(topright) -- [boson] (Zt),
				(bottomright) -- [boson] (Zb),
			};

			\node [below left=2.5cm and 1cm of gt] (B) {B};
		\end{feynman}
		\end{tikzpicture}
	\captionof{figure}
...
% \end{table}
% For reference, \cref{tab:cms_vs_atlas} summarizes the previous Higgs boson mass measurements made by ATLAS and CMS collaborations, using the \htofourl decay channel.
